...
def multi_head(out_dim, dropoutrate, lay_num, loss1, loss2):
    ...
    model = Model([text_input_c1, text_input_c2], outputs=[out1, out2])  # 入力と出力を設定し、Modelクラスでモデルを作成
...
def bilstmc2_model(out_dim, dropoutrate, lay_num):
    encoder_input = Input(shape=(39,)) # encoderの入力層
    decoder_input = Input(shape=(230,)) # decoderの入力層
