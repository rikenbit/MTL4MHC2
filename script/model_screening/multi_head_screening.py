...
def multi_head(out_dim, dropoutrate, lay_num):
    pep_input = Input(shape=(39,)) 
    mhc_input = Input(shape=(230,)) 
    
    pep_emb = Embedding(47, 100, input_length=39, weights=[monovec], trainable=False)(pep_input)
    mhc_emb = Embedding(9419, 100, input_length=230, weights=[trivec1_MHC], trainable=False)(mhc_input)
    
    # peptide
    pep_output1 = MultiHead(
    ...
    peptide_output1 = keras.layers.Flatten(name='Flatten1')(pep_output1)
    
    # MHC
    mhc_output1 = MultiHead(
        layer=keras.layers.Bidirectional(keras.layers.LSTM(out_dim, dropout=dropoutrate), name='LSTM'),
        layer_num=lay_num,
        reg_index=[1, 4],
        reg_slice=(slice(None, None), slice(128, 96)),
    ...
    MHC_output1 = keras.layers.Flatten(name='Flatten2')(mhc_output1)
    ...
    for i in out_dim:
        for k in lay_num:
            for l in dropoutrate:
                model = multi_head(out_dim=i, dropoutrate=l, lay_num=k)
